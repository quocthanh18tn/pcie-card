...
static int adxcvr_probe(struct platform_device *pdev)
{
	...
	/*Check ScratchPad Reg */
	adxcvr_write(st, ADXCVR_REG_SCRATCH, 0xABCD);
	unsigned int ScratchPad_Value = adxcvr_read(st, ADXCVR_REG_SCRATCH);
	pr_notice("ScratchPad_Value = %X\n", ScratchPad_Value);
	...
	pr_notice("adxcvr_probe : %s successfully\n", pdev->dev.of_node->name);
	return 0;

disable_unprepare:
	clk_disable_unprepare(st->conv_clk);

	pr_notice("adxcvr_probe : %s failed with ret = %d\n", pdev->dev.of_node->name, ret);
	return ret;
}
