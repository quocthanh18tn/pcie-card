...
static int adrv9009_do_setup(struct adrv9009_rf_phy *phy)
{
	...
	if (ret != TALACT_NO_ACTION) {
		if(ret == TALACT_NO_ACTION){
			pr_notice("TALACT_NO_ACTION\n");
		}
		if(ret == TALACT_WARN_RESET_LOG){
			pr_notice("TALACT_WARN_RESET_LOG\n");
		}
		if(ret == TALACT_ERR_CHECK_PARAM){
			pr_notice("TALACT_ERR_CHECK_PARAM\n");
		}
		if(ret == TALACT_ERR_RESET_SPI){
			pr_notice("TALACT_ERR_RESET_SPI\n"); 
		}
		if(ret == TALACT_ERR_RESET_ARM){
			pr_notice("TALACT_ERR_RESET_ARM\n");
		}
		if(ret == TALACT_ERR_CHECK_TIMER){
			pr_notice("TALACT_ERR_CHECK_TIMER\n");
		}
		if(ret == TALACT_ERR_RERUN_INIT_CALS){
			pr_notice("TALACT_ERR_RERUN_INIT_CALS\n");
		}
		pr_notice("errorFlag = %d\n", errorFlag);

		
		ret = TALISE_getInitCalStatus(phy->talDevice, &calsSincePowerUp, &calsLastRun, &calsMinimum, &initErrCal, &initErrCode);
		...
		pr_notice("calsSincePowerUp = 0x%X\n", calsSincePowerUp);
		pr_notice("calsLastRun = 0x%X\n", calsLastRun);
		pr_notice("calsMinimum = 0x%X\n", calsMinimum);
		pr_notice("initErrCal = 0x%X\n", initErrCal);
		pr_notice("initErrCode = 0x%X\n", initErrCode);
	...
	TALISE_setRadioCtlPinMode(phy->talDevice, TAL_TXRX_PIN_MODE | TAL_ORX_PIN_MODE, TAL_ORX1ORX2_PAIR_01_SEL);
	adrv9009_sysref_req(phy, SYSREF_CONT_ON);
				 ...
				 char *buf)
{
	...
	case ADRV9009_INIT_CAL:
		...
		if (val)
			ret = sprintf(buf, "%d\n", !!(phy->cal_mask & val));
		break;
	case ADRV9009_PLLSTATUS:
		TALISE_getPllsLockStatus(phy->talDevice, &status);
...
static IIO_DEVICE_ATTR(pllstatus, S_IRUGO ,
		       adrv9009_phy_show,
		       NULL,
		       ADRV9009_PLLSTATUS);
...
static struct attribute *adrv9009_phy_attributes[] = {
	...
	&iio_dev_attr_pllstatus.dev_attr.attr,
	NULL,
};
...
#define ADRV9009_GET_PROFILE(_dt_name, _member) \
	...
	if (ret < 0) { \
		dev_err(dev, "9009 %s Failed to read %u coefficients\n", _dt_name, (u32) ARRAY_SIZE(_member)); \
		return ret; \
	} \
...
static int adrv9009_probe(struct spi_device *spi)
{
	...
	if (IS_ERR(clk)){
		printk("devm_clk_get failed");
		return PTR_ERR(clk);
	}

	printk("ADRV9009 probe devm_iio_device_alloc");
	...
	printk("ADRV9009 probe adrv9009_phy_parse_d");
	ret = adrv9009_phy_parse_dt(indio_dev, &spi->dev);
	if (ret < 0){
		printk("ADRV9009 probe adrv9009_phy_parse_d failed");
		return -ret;
	}
	...
	phy->sysref_req_gpio = devm_gpiod_get(&spi->dev, "sysref-req",
					      GPIOD_OUT_HIGH);
	gpiod_export(phy->sysref_req_gpio, false);
	orx2_enable = devm_gpiod_get(&spi->dev, "orx2-enable",
					      GPIOD_OUT_LOW);
	gpiod_direction_output(orx2_enable, 0);
	gpiod_export(orx2_enable, false);
	...
	printk("ADRV9009 probe clock jesd_tx_clk");
	if (id == ID_ADRV9009)
		phy->jesd_tx_clk = devm_clk_get(&spi->dev, "jesd_tx_clk");
		pr_notice("jesd_tx_clk name = %s\n", __clk_get_name(phy->jesd_tx_clk));
		struct clk_hw *jesd_tx_clk_hw = __clk_get_hw(phy->jesd_tx_clk);
		pr_notice("jesd_tx_clk_hw num_parents = %d\n", clk_hw_get_num_parents(jesd_tx_clk_hw));
		struct clk_hw *jesd_tx_clk_parents_hw = clk_hw_get_parent_by_index(jesd_tx_clk_hw,0);
		pr_notice("jesd_tx_clk_parents name = %s\n", clk_hw_get_name(jesd_tx_clk_parents_hw));

	printk("ADRV9009 probe clock jesd_rx_os_clk");
	if (id == ID_ADRV9009 || id == ID_ADRV90082)
		phy->jesd_rx_os_clk = devm_clk_get(&spi->dev, "jesd_rx_os_clk");

	printk("ADRV9009 probe clock dev_clk");
	phy->dev_clk = devm_clk_get(&spi->dev, "dev_clk");
	if (IS_ERR(phy->dev_clk))
		return PTR_ERR(phy->dev_clk);

	printk("ADRV9009 probe clock fmc_clk");
	phy->fmc_clk = devm_clk_get(&spi->dev, "fmc_clk");
	if (IS_ERR(phy->fmc_clk))
		return PTR_ERR(phy->fmc_clk);

	printk("ADRV9009 probe clock sysref_clk");
	...
	printk("ADRV9009 probe arm-firmware-name");
	...
	printk("ADRV9009 probe request_firmware");
	...
	printk("ADRV9009 probe stream-firmware-name");
	...
	printk("ADRV9009 probe adrv9009_setup");
	ret = adrv9009_setup(phy);
	if (ret < 0) {
		/* Try once more */
		ret = adrv9009_setup(phy);
