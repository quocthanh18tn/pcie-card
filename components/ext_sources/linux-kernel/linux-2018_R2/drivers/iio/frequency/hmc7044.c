...
#define HMC7044_STATUP_MODE_CONFIG(x) (((x) & 0x3) << 2)
#define HMC7044_STATUP_MODE_ASY 0
#define HMC7044_STATUP_MODE_DYN 3
...
#define HMC7044_DYNAMIC_DRIVER_EN BIT(5)
#define HMC7044_FORCE_MUTE(x) (((x) & 0x3) << 6)	
#define HMC7044_FORCE_MUTE_NORMAL			0
#define HMC7044_FORCE_MUTE_LOGIC_0			2
...
struct gpio_desc	*switch_hmc7044;
struct gpio_desc	*reset_hmc7044;

#define to_output(_hw) container_of(_hw, struct hmc7044_output, hw)

#define SPI0_SDIO_SS_GPIO 	95
void __spi_3wire_prepare_tranfer(int mode){
    gpiod_direction_output(switch_hmc7044, !mode);
    udelay(5);	
}
...
static int hmc7044_read(struct iio_dev *indio_dev, unsigned reg)
{
	struct hmc7044 *hmc = iio_priv(indio_dev);
	int ret;
			      ...
			      unsigned int writeval, unsigned int *readval)
{
	...
	mutex_lock(&hmc->lock);
	if (readval == NULL) {
		hmc7044_write(indio_dev, reg, writeval);
		ret = 0;
	} else {
		*readval = hmc7044_read(indio_dev, reg);
		ret = 0;
	}
	...
	for (i = 0; i < hmc->num_channels; i++) {
		...
		if (hmc->channels[i].startup_mode == 0) {
			hmc7044_write(indio_dev, HMC7044_REG_CH_OUT_CRTL_0(chan->num),
			      HMC7044_HI_PERF_MODE | HMC7044_SYNC_EN | HMC7044_STATUP_MODE_CONFIG(HMC7044_STATUP_MODE_ASY) |
			      HMC7044_CH_EN);			
		} else if (hmc->channels[i].startup_mode == 1) {
			hmc7044_write(indio_dev, HMC7044_REG_CH_OUT_CRTL_0(chan->num),
			      HMC7044_HI_PERF_MODE | HMC7044_SYNC_EN | HMC7044_STATUP_MODE_CONFIG(HMC7044_STATUP_MODE_DYN) |
			      HMC7044_CH_EN);
		}
		...
		if (hmc->channels[i].startup_mode == 0) {
			hmc7044_write(indio_dev, HMC7044_REG_CH_OUT_CRTL_8(chan->num),
			      HMC7044_DRIVER_MODE(chan->driver_mode) | HMC7044_FORCE_MUTE(HMC7044_FORCE_MUTE_NORMAL));

		} else if (hmc->channels[i].startup_mode == 1) {
			hmc7044_write(indio_dev, HMC7044_REG_CH_OUT_CRTL_8(chan->num),
			      HMC7044_DRIVER_MODE(chan->driver_mode) | HMC7044_FORCE_MUTE(HMC7044_FORCE_MUTE_LOGIC_0));
		}		
...
static int hmc7044_probe(struct spi_device *spi)
{
	...
	pr_notice("viettel_hmc7044_probe \n");
	...
	switch_hmc7044 = devm_gpiod_get(&spi->dev, "switch", GPIOD_OUT_LOW);
	reset_hmc7044 = devm_gpiod_get(&spi->dev, "reset", GPIOD_OUT_LOW);

	if(switch_hmc7044){
		ret_export = gpiod_export(switch_hmc7044,false);
		if(ret_export<0){
			pr_notice("Export switch_hmc7044 pin failed with ret = %d", ret_export);
		}
		else{
			pr_notice("Export switch_hmc7044 pin successfully\n");
		}
	}
	else{
		pr_notice("Descriptor of switch_hmc7044 pin is NULL\n");
	}

	if(reset_hmc7044){
		ret_export = gpiod_export(reset_hmc7044,false);
		if(ret_export<0){
			pr_notice("Export reset_hmc7044 pin failed with ret = %d", ret_export);
		}
		else{
			pr_notice("Export reset_hmc7044 pin successfully\n");
		}
	}
	else{
		pr_notice("Descriptor of reset_hmc7044 pin is NULL\n");
	}

	spi_set_drvdata(spi, indio_dev);
	spi->prepare_transfer = __spi_3wire_prepare_tranfer;
	...
	pr_notice("viettel_hmc7044_probe sucessful \n");
