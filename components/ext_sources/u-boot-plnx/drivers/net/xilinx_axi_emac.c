...
static int setup_phy(struct udevice *dev)
{
	...
	ret = phyread(priv, priv->phyaddr, MII_PHYSID1, &temp);
	if (ret)
		return 0;
	printf("Get PHY identifier at %d: %d\n", MII_PHYSID1, temp);

	ret = phyread(priv, priv->phyaddr, MII_PHYSID2, &temp);
	if (ret)
		return 0;
	printf("Get PHY identifier at %d: %d\n", MII_PHYSID2, temp);
			       ...
			       int devad, int reg)
{
	...
	printf("axiemac: Read MII 0x%x, 0x%x, 0x%x, %d\n", addr, reg,
	      value, ret);
	return value;
}
...
static int axiemac_miiphy_write(struct mii_dev *bus, int addr, int devad,
				int reg, u16 value)
{
	printf("axiemac: Write MII 0x%x, 0x%x, 0x%x\n", addr, reg, value);
